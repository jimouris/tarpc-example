...
async fn main() -> anyhow::Result<()> {
    let server_addr_0 = SocketAddr::new("0.0.0.0".parse().expect("Could not parse"), 8000);

    let transport0 = tarpc::serde_transport::tcp::connect(server_addr_0, Json::default);

    let client0 = WorldClient::new(
        client::Config::default(), transport0.await?
    ).spawn();

    let hello = client0.hello(context::current(), "Stim".to_string()).await?;
    println!("{hello}");
