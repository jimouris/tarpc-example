...
impl World for HelloServer {
    ...
    async fn server_to_server(self, _: context::Context) -> String {
        let transport = tarpc::serde_transport::tcp::connect(self.server_addr, Json::default).await.unwrap();
        ...
        let hello = client.hello(context::current(), "YOOO".to_string()).await.unwrap();
    ...
    listener
        ...
        .map(|channel| { 
            let server = HelloServer {
                server_addr: SocketAddr::new("0.0.0.0".parse().expect("Could not parse"), 8001),
            };
